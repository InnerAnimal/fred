// Footer JavaScript - Meauxbility
(function() {
  'use strict';

  // Theme Management
  const darkModePages = ['about', 'community', 'resources'];
  const lightModePages = ['programs', 'connect', 'impact', 'home'];
  ...
  function initializeTheme() {
    ...
    lightModePages.forEach(page => {
      if (currentPath.includes(page) || currentPath === '/fred/' || currentPath === '/') pageTheme = 'light';
    });
  ...
  class MbxPaymentController {
    ...
    async init() {
      if (typeof Stripe !== 'undefined') {
        ...
        const stripeKey = process?.env?.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || 'pk_live_51S4R0SRW56Pm3uYI8EKbysm1ok4peVXSD6G17HtFy8BDuG9Carn8Ry7iPVzulMBtdEFcz5pFvXpE04CIgn8PY6WS00aXOqMYEI';
